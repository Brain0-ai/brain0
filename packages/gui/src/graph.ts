...
import type { GraphSnapshot, GraphNode, Level } from "@brain0/shared";
...
export class ForceLayout {
  ...
  constructor(
    ...
    private readonly params: ForceParams = DEFAULT_FORCES,
  ) {
   ...
   */
  relax(minDist: number, iterations: number): void {
    const n = this.nodes.length;
    for (let it = 0; it < iterations; it++) {
      ...
      if (!moved) break; // converged — no overlaps remain
    }
    this.recenter();
  }
...
export function lodLevelForZoom(zoom: number): Level {
  if (zoom < 0.6) return "repo";
  if (zoom < 1.2) return "module";
  if (zoom < 2.4) return "file";
  return "symbol";
}

/** Nodes visible at a given zoom: artifacts at the current LOD level; tasks once zoomed in. */
export function visibleNodes(snapshot: GraphSnapshot, zoom: number): GraphNode[] {
  const level = lodLevelForZoom(zoom);
  return snapshot.nodes.filter((node) =>
    node.kind === "task" ? zoom >= 1.2 : node.level === level,
  );
}

/** The set of node ids to highlight given the tasks the agent surfaced. */
export function highlightSet(snapshot: GraphSnapshot, taskIds: string[]): Set<string> {
  const tasks = new Set(taskIds);
  const highlight = new Set<string>(taskIds);
  for (const edge of snapshot.edges) {
